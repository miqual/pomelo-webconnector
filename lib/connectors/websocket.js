 ...
 * @auth Guofeng.Ding
 */
var socket = function( id, socket,timeOut,tracerMsg){
    ...
    this.timeOutId =setTimeout(function(){
        self.socket.status(403).send({
            success: false, 
            error: 'request timeout'
        });

        tracerMsg = tracerMsg && JSON.stringify(tracerMsg);
        logger.error('timeout, id ',self.id,' detail ',tracerMsg);
    },timeOut);
...
socket.prototype.send = function( msg ){
    if( this.state == ST_CLOSED ){
        logger.warn('response is closed ',JSON.stringify(msg));
        return;
    }
